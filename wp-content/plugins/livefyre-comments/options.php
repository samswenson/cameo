...
if (get_option('livefyre_import_status','')=='started' && !$new_import) {
	...
	if (get_option('livefyre_import_status','') == 'activity_map_error') {
		$printthis = "Restarting failed import session...";
		if (livefyre_new_import_session(true)) {
			?>
			<script type="text/javascript">
				livefyre_start_ajax();
			</script>
			...
			$printthis = "Error requesting livefyre import session.";
		}
	} else if (get_option('livefyre_active_sync','')=='1') {
...
<p id="show_import_queue" style="display:none;">Your WordPress import request has been queued, and will be processed by our server in the order it was received.  This page will update frequently with the latest status, but it may take a few minutes and you are free to leave this page during the process.</p>
</div>
